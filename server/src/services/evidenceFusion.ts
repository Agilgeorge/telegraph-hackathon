import { NormalizedSignal, Claim } from '../types/verification';
...
export function fuseEvidence(signals: NormalizedSignal[], claims: Claim[]): EvidenceItem[] {
  ...
  // Fraud signals
  const fraudSignals = signals.filter(
    (s) => s.intent === 'FRAUD_DETECTION' && NEGATIVE_VERDICTS.has(s.verdict)
  );
  ...
  // URL scan
  const urlScanNegative = signals.filter(
    (s) => s.intent === 'URL_SCAN' && NEGATIVE_VERDICTS.has(s.verdict)
  );
  ...
  // Fact check failed
  const factCheckNegative = signals.filter(
    (s) => s.intent === 'FACT_CHECK' && NEGATIVE_VERDICTS.has(s.verdict)
  );
...
export function addAvailabilityEvidence(
  evidence: EvidenceItem[],
  failedCount: number,
  expectedCount: number
): EvidenceItem[] {
  if (failedCount === 0) return evidence;
  return [
    ...evidence,
    {
      type: 'warning',
