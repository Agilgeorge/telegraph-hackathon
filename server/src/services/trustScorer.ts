import { NormalizedSignal, Claim, RiskLevel, ActionRisk } from '../types/verification';
...
const ACTION_RISK_PENALTY: Record<ActionRisk, number> = {
  read: 0,
  download: -10,
  login: -15,
  financial: -25,
...
export function calculateTrustScore(
  ...
  for (const s of negativeSignals) {
    if (s.confidence > 0.8) score -= 10;
  }

  // Evidence penalties
  for (const e of evidence) {
    if (e.type === 'injection') score -= 50;
    else if (e.type === 'contradiction') score -= 15;
    else if (e.type === 'warning') score -= 10;
  }
