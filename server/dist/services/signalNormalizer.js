...
function extractVerdict(signal, intent) {
    ...
    if (typeof s.verdict === 'string')
        return s.verdict;
    if (typeof s.risk_tier === 'string')
        return s.risk_tier;
    if (typeof s.risk_level === 'string')
        return s.risk_level;
    if (typeof s.label === 'string')
        return s.label;
    if (typeof s.status === 'string')
        return s.status;
    if (typeof s.assessment_status === 'string')
        return s.assessment_status;
    ...
    if (intent === 'FRAUD_DETECTION') {
        if (typeof s.is_suspicious === 'boolean')
            return s.is_suspicious ? 'suspicious' : 'clean';
    ...
    if (intent === 'FACT_CHECK') {
        if (typeof s.supported === 'boolean')
            return s.supported ? 'supported' : 'unsupported';
    }
    return 'unknown';
}
function extractConfidence(signal, response) {
...
function normalizeSignal(response) {
    return {
